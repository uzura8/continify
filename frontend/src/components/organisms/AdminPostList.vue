        ...
        <div class="">
          <div class="flex-container">
            <FormSelectField
              ...
              :option-objs="filterAttributeOptions"
              :label-text="$t('common.target')"
              :is-width-full="true"
            />
            <FormSelectField
              class="flex-item"
              v-model="filterCompare"
              :option-objs="filterCompareOptions"
              :label-text="$t('common.condition')"
              :is-width-full="true"
            />
      ...
      <table class="table is-fullwidth mt-6">
        <thead>
          <tr>
            ...
            <th class="is-size-7 u-clickable is-underlined">
              <router-link :to="getUrlObjBySortOrder('publishAt')">
            ...
            <th class="is-size-7 u-clickable is-underlined">
              <router-link :to="getUrlObjBySortOrder('createdAt')">
...
export default {
  ...
  data() {
    return {
      ...
      filterAttributeOptions: [
        { label: this.$t('form.slug'), value: 'slug' },
        { label: this.$t('form.title'), value: 'title' },
        { label: this.$t('form.body'), value: 'body' }
      ],
      filterCompareOptions: [
        { label: this.$t('common.equalTo'), value: 'eq' },
  ...
  computed: {
    ...
    filtersQuery() {
      ...
      if (
        utilObj.checkObjItemsNotEmpty(reqFilters, ['value', 'attribute', 'compare'], true) === false
      ) {
        return defaultValue
      }
      return reqFilters
    ...
    filtersInput() {
      ...
      if (
        utilObj.checkObjItemsNotEmpty(inputs, ['value', 'attribute', 'compare'], true) === false
      ) {
        return defaultValue
      }
      return inputs
    ...
    filtersJson() {
      ...
      if (
        utilObj.checkObjItemsNotEmpty(this.filters, ['value', 'attribute', 'compare'], true) ===
        false
      ) {
        return defaultValue
      }
    ...
    currentPagerKey() {
      ...
      const current = this.$store.state.adminPostsPager.keys.find(
    ...
    currentParams() {
      ...
      if (this.filterCategoryForReq) {
        params.category = this.filterCategoryForReq
      }
      if (this.filters) {
    ...
    currentParamsForReq() {
      ...
      } else if (this.filterCategoryQuery) {
        params.category = this.filterCategoryQuery
      }

      if (this.filters) {
        params.filters = JSON.stringify(this.filters)
      } else if (this.filtersQuery) {
        params.filters = JSON.stringify(this.filtersQuery)
      }
  ...
  watch: {
    ...
    filtersQuery: {
      handler(val, oldVal) {
        // オブジェクトが存在しない、または変更がない場合、処理を中止
        if (
          !val ||
          (oldVal &&
            val.attribute === oldVal.attribute &&
            val.compare === oldVal.compare &&
            val.value === oldVal.value)
        ) {
          return // 何も変更がないため、ここで処理を中止
        }
  ...
  async created() {
    ...
    this.fetchPosts(null, isReqLatestPosts)
  },
  ...
  methods: {
    checkReqIndexAndRedirect() {
      if (this.validateIndex() === false) {
        ...
        this.$router.push({
          ...
          query: this.currentParamsForReq
        })
      }
    },
    ...
    checkParamsChanged() {
      ...
      if (Object.prototype.hasOwnProperty.call(current, 'filters') && current.filters != null) {
