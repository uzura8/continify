    ...
    >
      ...
      <template v-else>
        <template v-for="(mess, i) in formattedMessage">
          ...
          <br
            :key="i"
            v-if="i + 1 < formattedMessage.length"
          />
        </template>
      </template>
    </p>
...
<script>
import config from '@/util/buefy/config'
import BFieldBody from './BFieldBody.vue'

export default {
  name: 'BField',
  components: {
    [BFieldBody.name]: BFieldBody
  },
  provide() {
    return {
      BField: this
  ...
  computed: {
      ...
      return [
        {
          ...
          'is-floating-in-label':
            this.hasLabel && !this.horizontal && this.labelPosition === 'inside',
          'is-floating-label':
            this.hasLabel && !this.horizontal && this.labelPosition === 'on-border'
        }
    ...
    hasInnerField() {
      ...
      return this.grouped || this.groupMultiline
    },
    /**
    ...
    hasMessage() {
      return (
        ((!this.parent || !this.parent.hasInnerField) && this.newMessage) || this.$slots.message
      )
    }
    //numberInputClasses() {
