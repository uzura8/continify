...
export default {
  ...
  async created() {
    this.categorySlug = this.modelValue

    // set value on parent
    if (this.modelValue) {
      // reset to initial value
      this.initialCategorySlug = this.modelValue
      // if set value on parent, set category object
      await this.setInputtedCategory()
    }
    if (this.modelValue) {
  ...
  methods: {
    async setInputtedCategory() {
      ...
      try {
        this.inputtedCategory = await Category.get(this.serviceId, this.initialCategorySlug, {
          withParents: 1
        })
