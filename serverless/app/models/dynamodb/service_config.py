...
class ServiceConfig(Base):
    ...
    alloweds = [
        ...
        },
        {
            ...
            'isJson': False,
            'default': 'unpublish',
            'allowed': ['unpublish', 'publish', 'unconfirmed'],
        },
    ]
    ...
    def get_alloweds(self):
        ...
        for item in self.alloweds:
            val = json.loads(
                item['default']) if item['isJson'] else item['default']
            res.append({
    ...
    def get_one_by_name(self, service_id, name, is_json_loads=True, supply_if_empty=False):
        ...
        res = table.query(
            KeyConditionExpression=Key('serviceId').eq(
                service_id) & Key('configName').eq(name)
        )
    ...
    def save(self, service_id, name, val):
        ...
        comp_val = json.dumps(
            item['configVal']) if item['isJson'] else item['configVal']
    ...
    @classmethod
    def increment_number(self, service_id, name):
        item = self.get_one_by_name(service_id, name)
        if not item:
            self.save(service_id, name, 1)
            return 1
        keys = {'serviceId': service_id, 'configName': name}
        item = self.update_add_values(keys, {'configVal': 1}, True)
        return item['configVal']
