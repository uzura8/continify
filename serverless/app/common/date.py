...
def iso_offset2utc(date_offset, conv_tz_spec=False):
    ...
    date_utc = dt.astimezone(tz=dt.tzinfo).astimezone(
...
def utc_str2local_str(utc_time_str, timezone_str, date_format='%Y/%m/%d %H:%M:%S'):
...
def validate_date_str(date_str, date_format='%Y-%m-%dT%H:%M:%S%z'):
    try:
