...
def post_list(service_id):
    ...
    if request.method == 'POST':
        ...
        pager_keys = {'pkey': 'postId',
                      'index_pkey': 'serviceId', 'index_skey': index_skey}
        post = Post.query_pager_admin(
            pkeys, vals, pager_keys, index, filter_conds)
...
def post_detail(service_id, identifer):
    ...
    if request.method == 'POST':
        ...
        if vals.get('tags', []):
            try:
                res = update_post_tags(
                    saved, vals['tags'], is_upd_status_publish_at)
                if res.get('current_ids'):
                    keys = []
                    for tag_id in res['current_ids']:
                        keys.append({'tagId': tag_id})
            ...
            try:
                post_tags = PostTag.get_all_by_post_id(
                    post['postId'], False, False)
                if post_tags:
                    del_tags = [{'tagId': pt['tagId'], 'postId': pt['postId']}
                                for pt in post_tags]
        ...
        if post.get('tags'):
            try:
                del_tags = [{'tagId': tag['tagId'], 'postId': post['postId']}
...
@bp.get('/posts/<string:service_id>/<string:identifer>/cache')
@cognito_auth_required
@admin_role_editor_required
def get_cache_info(service_id, identifer):
    check_acl_service_id(service_id)
    post = get_post_by_identifer(service_id, identifer)

    s3handler = AwsS3Handler(MEDIA_S3_BUCKET_NAME, MEDIA_DISTRIBUTION_ID)
    path = f'posts/{service_id}/{post["slug"]}.json'
    uploaded_at = s3handler.get_file_uploaded_at(path)
    res = None
    if uploaded_at:
        res = {'path': path, 'uploadedAt': uploaded_at}
        # raise InvalidUsage('Not Found', 404)
    return jsonify(res), 200
...
def update_post_tags(post, req_tags, is_update_status_publish_at=False):
    ...
    if new_tag_labels:
        ...
        for ntl in new_tag_labels:
            ...
            if ntl in exist_tag_labels:
                tag_id = [d['tagId']
                          for d in exist_tags if d['label'] == ntl][0]
    ...
    saved_post_tags = PostTag.get_all_by_pkey(
        {'key': 'postId', 'val': post['postId']})
    if saved_post_tags:
    ...
    if del_tag_ids:
        del_tags = [{'tagId': tid, 'postId': post['postId']}
                    for tid in del_tag_ids]
        PostTag.batch_delete(del_tags)
